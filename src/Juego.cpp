...
void Juego::updateFondo() {
    ...
    //Colocar Arma en el centro
    if(posY / 820.f == 1 && !stopArmas) {
        srand(time(NULL));
        int tipoArma = (rand() % (3 - 1)) + 1;
        vectorArmas.push_back(factoriaArma.crearArma(tipoArma, 1));
    }

    //Colocar Arma en la derecha
    if(posY / 820.f == 2 && !stopArmas) {
        srand(time(NULL));
        int tipoArma = (rand() % (3 - 1)) + 1;
        vectorArmas.push_back(factoriaArma.crearArma(tipoArma, 2));
    }

    //Colocar Arma en la izquierda
    if(posY / 820.f == 3 && !stopArmas) {
        srand(time(NULL));
        int tipoArma = (rand() % (3 - 1)) + 1;
